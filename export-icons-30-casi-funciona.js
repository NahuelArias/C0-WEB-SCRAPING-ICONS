...
const DEFAULT_CONFIG = Object.freeze({
    ...
    outputDir: "./icons",
    defaultSize: 48,
    defaultColor: "#000000", // Cambiado de currentColor a color hexadecimal por defecto
    formats: ['svg'],
    fileNaming: {
...
class ImageConverter {
    async convert(svgContent, format, size, backgroundColor = null) {
        const targetFormat = format === 'jpg' ? 'jpeg' : format;
        
        try {
            const options = {
                density: Math.max(size * 2, 72)
            };
            
            if (backgroundColor && format !== 'svg') {
                // Añadir fondo si se especifica (útil para PNG/JPEG)
                options.background = backgroundColor;
            }
            
            const image = sharp(Buffer.from(svgContent), options);
...
class FileHandler {
    ...
    generateFileName(config, collection, iconName, options = {}) {
        ...
        let fileName = config.fileNaming.pattern
            ...
            .replace(/{color}/g, color.replace('#', 'hex') || 'default')
    ...
    generateFolderPath(config, collection, options = {}) {
        ...
        if (config.folderStructure.groupByColor && color && color !== config.defaultColor) {
            fullPath = path.join(fullPath, `color-${this.sanitizeString(color.replace('#', 'hex'), config)}`);
        }
...
class SvgProcessor {
    ...
    generateSvg(data, iconName, collection, size, color) {
        try {
            ...
            // Configuración para iconToSVG
            const options = {
                height: `${size}`,
                width: `${size}`
            };
            
            // Solo pasar color si es un hex válido y no es currentColor
            if (targetColor !== 'currentColor' && isValidHexColor(targetColor)) {
            ...
            // Aplicar color manualmente si es necesario
            if (targetColor !== 'currentColor' && isValidHexColor(targetColor)) {
                // Reemplazar todos los fill="none" o fill sin valor con nuestro color
                body = body.replace(/fill="([^"]*)"/g, (match, fillValue) => {
                    // Si el fill no es "none" y tiene un valor, mantenerlo
                    if (fillValue !== 'none' && fillValue !== '') {
                        return match;
                    }
                    // Si es "none" o vacío, aplicar nuestro color
                    return `fill="${targetColor}"`;
                });
                
                // Si no hay atributo fill, añadirlo al primer path
                if (!/<path[^>]*fill=/i.test(body)) {
                    body = body.replace(/<path/g, `<path fill="${targetColor}"`);
                }
                
                // También manejar stroke si existe
                body = body.replace(/stroke="([^"]*)"/g, (match, strokeValue) => {
...
class IconExporter {
    ...
    validateConfig() {
        ...
        // Validar color por defecto
        if (this.config.defaultColor !== 'currentColor' && !isValidHexColor(this.config.defaultColor)) {
            console.warn(`⚠️ Color por defecto "${this.config.defaultColor}" no es un color hexadecimal válido. Usando #000000`);
            this.config.defaultColor = '#000000';
        }
    }
    ...
    async processIcon(collectionData, iconName, collection, options = {}) {
        ...
        // Verificar que el SVG tenga el color correcto
        if (targetColor !== 'currentColor' && isValidHexColor(targetColor)) {
            if (!svgContent.includes(`fill="${targetColor}"`) && 
                !svgContent.includes(`fill='${targetColor}'`) &&
                !svgContent.includes(`fill:${targetColor}`)) {
                console.warn(`⚠️ El color ${targetColor} podría no haberse aplicado a ${iconName}`);
            }
        }
        ...
        for (const format of this.config.formats) {
            try {
                ...
                if (format !== 'svg') {
                    // Para formatos raster, podemos añadir fondo blanco para mejor visibilidad
                    const backgroundColor = format === 'png' || format === 'jpeg' ? '#FFFFFF' : null;
                    fileContent = await this.imageConverter.convert(
                        svgContent, 
                        format, 
                        targetSize,
                        backgroundColor
    ...
    async processCollection(collection, variants = {}) {
        ...
        try {
            ...
            console.log(`   Iconos: ${icons.length}`);
            console.log(`   Formatos: ${this.config.formats.join(', ')}`);
            console.log(`   Tamaños: ${sizes.join(', ')}`);
            console.log(`   Colores: ${colors.map(c => c === 'currentColor' ? c : c.toUpperCase()).join(', ')}`);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === fileURLToPath(`file://${process.argv[1]}`)) {
    ...
    // CONFIGURACIÓN CORRECTA CON COLORES HEXADECIMALES
    const config = {
        collections: ["mdi"], // Material Design Icons
        outputDir: "./icon-exports",
        defaultSize: 24,
        defaultColor: "#1d9bf0", // AZUL - color hexadecimal
        iconsToExport: ["home", "account", "settings", "heart"], 
        formats: ['svg', 'png'],
        fileNaming: {
            pattern: "{icon}-{color}",
            case: "kebab"
    ...
    // VARIANTES CON COLORES HEXADECIMALES EXPLÍCITOS
    const variants = {
        sizes: [24, 32],
        colors: [
            "#1d9bf0",  // Azul Twitter
    ...
    try {
        ...
        if (result.success) {
            ...
            console.log('📁 Los iconos se encuentran en:', path.resolve(config.outputDir));
        } else {
