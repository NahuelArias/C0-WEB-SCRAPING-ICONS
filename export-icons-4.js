...
const DEFAULT_CONFIG = {
    ...
    fileNaming: {
        pattern: "{collection}-{icon}", // Patrón: {collection}, {icon}, {size}, {color}
        extension: "svg",
        sanitize: true, // Limpia caracteres especiales
        case: "kebab" // "kebab", "camel", "pascal", "snake", "original"
    },
    folderStructure: {
        enabled: true, // Si false, todos los archivos van a outputDir directamente
        pattern: "{collection}", // Patrón: {collection}, {collection}/{size}, custom path
...
class IconExporter {
    ...
    /**
     * Convierte caracteres especiales al alfabeto español
     */
    convertToSpanishAlphabet(str) {
        const charMap = {
            'ä': 'a', 'æ': 'a', 'å': 'a', 'à': 'a', 'á': 'a', 'â': 'a', 'ã': 'a', 'ā': 'a',
            'ç': 'c', 'č': 'c', 'ć': 'c',
            'đ': 'd', 'ð': 'd',
            'ë': 'e', 'è': 'e', 'é': 'e', 'ê': 'e', 'ē': 'e', 'ę': 'e', 'ě': 'e',
            'ğ': 'g',
            'ï': 'i', 'î': 'i', 'ì': 'i', 'í': 'i', 'ī': 'i', 'ı': 'i',
            'ł': 'l',
            'ñ': 'n', 'ń': 'n', 'ň': 'n',
            'ö': 'o', 'ø': 'o', 'ò': 'o', 'ó': 'o', 'ô': 'o', 'õ': 'o', 'ō': 'o', 'ő': 'o',
            'ř': 'r',
            'š': 's', 'ş': 's', 'ś': 's',
            'þ': 'th',
            'ü': 'u', 'ù': 'u', 'ú': 'u', 'û': 'u', 'ū': 'u', 'ů': 'u', 'ű': 'u',
            'ÿ': 'y', 'ý': 'y',
            'ž': 'z', 'ż': 'z', 'ź': 'z',
            'ß': 'ss',
            'œ': 'oe',
    ...
    sanitizeString(str) {
        ...
        // Primero convertir caracteres especiales al alfabeto español
        let sanitized = this.convertToSpanishAlphabet(str);
        
        // Luego aplicar las demás sanitizaciones
        return sanitized
    ...
    async processCollection(collection) {
        try {
            ...
            for (const iconName of icons) {
                ...
                // Procesar con configuración base
        ...
        try {
            ...
            for (const collection of this.config.collections) {
                ...
                for (const iconName of icons) {
                    ...
                    // Procesar todas las combinaciones de tamaño y color
                ...
                totalIcons += icons.length * sizes.length * colors.length;
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/'))) {
    ...
    const customConfig = {
        collections: ["nonicons"],
        outputDir: "./exported-icons",
        fileNaming: {
            pattern: "{icon}-{size}",
            case: "pascal" // "kebab", "camelCase", "PascalCase", "snake", "original"
        },
        folderStructure: {
            enabled: true,
            pattern: "{collection}",
